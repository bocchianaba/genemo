...
import { Town, User } from '../shared/models/user.models';
...
export class ModuleComponent implements OnInit, OnDestroy {
  ...
  go_to_city(event: any){
    console.log("event ", event)
    this.router.navigateByUrl('/feature/city/'+event.value+'/modules')
  }

  ngOnInit(): void {
    this.route.params.subscribe(params => {
      this.region_name = params['region_name'];
      this.loader.startBackground()
      this.store.select('user').subscribe(user=> this.cities=user.idTownAuthorise)
      this.region_name=this.route.snapshot.paramMap.get('region_name')??''
      this.modules$=this.module_service.get_all_modules(this.region_name,1,this.limit)
  ...
  page_change(page:number){
    ...
    this.loader.startBackground()
    this.subscription.unsubscribe()
    this.modules$=this.module_service.get_all_modules(this.region_name,page,this.limit,this.search_name)
